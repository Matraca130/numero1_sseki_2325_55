...
## FIXED IN THIS SESSION (3 commits)
...
| **PN-3** | `useReadingTimeTracker.ts:~78` | **Unguarded console.warn**: `console.warn('[ReadingTimeTracker] Save failed:', err)` leaks to production | Wrap in `import.meta.env.DEV` | XS |
...
| **PN-9** | `useFlashcardNavigation.ts` | **enrichedTopicCache never cleared**: Ref holds growing Map, never evicted on course change | Clear in the `useEffect` that resets on `currentCourse.id` | XS |
...
| **PN-15** | `useKeywordNavigation.ts:~152` | **`catch (err: any)` remnant**: Should be `catch (err: unknown)` per migration | Change to `unknown` + type guard | XS |
...
1. **PN-3** (XS) — Guard `useReadingTimeTracker` console.warn
2. **PN-12** (XS) — Guard PlatformDataContext refresh console.errors
3. **PN-9** (XS) — Clear enrichedTopicCache on course change
