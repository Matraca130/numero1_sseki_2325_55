...
export function RoundHistoryList({ rounds, title = 'Resumen por ronda', bare = false }: RoundHistoryListProps) {
  if (rounds.length <= 1) return null;
  const content = (
    <>
      <h4 className="text-xs text-gray-400 mb-2" style={{ fontWeight: 600 }}>{title}</h4>
          ...
          return (
            <div key={round.roundNumber} className="flex items-center justify-between text-xs text-gray-500 px-2 py-1">
              <div className="flex items-center gap-1.5">
                ...
                <span>Ronda {round.roundNumber}{round.source === 'ai' && <span className="text-violet-400"> (IA)</span>}</span>
              </div>
  ...
  return <div className="w-full bg-white/60 backdrop-blur-sm rounded-xl border border-gray-200/60 p-3">{content}</div>;
}
