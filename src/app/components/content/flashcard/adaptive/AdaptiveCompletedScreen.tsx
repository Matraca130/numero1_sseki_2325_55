  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col items-center justify-center p-8 text-center relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center max-w-md">
        ...
        <p className="text-sm text-gray-500 mb-6">{totalReviews} flashcards en {totalRounds} {totalRounds === 1 ? 'ronda' : 'rondas'}{aiRounds > 0 && <span className="text-violet-500"> ({profRounds} profesor + {aiRounds} IA)</span>}</p>
        <div className="mb-8"><MasteryRing value={masteryPct / 100} size={120} stroke={8} color={masteryColor.hex} /><p className="text-xs text-gray-500 mt-2" style={{ fontWeight: 500 }}>Dominio general</p></div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onExit} className="px-7 py-2.5 rounded-xl bg-teal-600 text-white text-sm shadow-lg shadow-teal-600/25 hover:bg-teal-700 transition-colors" style={{ fontWeight: 600 }}>Finalizar</button>
        </div>
      </div>
    </motion.div>
  );
