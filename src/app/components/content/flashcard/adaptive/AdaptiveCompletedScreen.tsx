...
//
// Extracted from AdaptiveFlashcardView.tsx (was CompletedScreen inline).
// Shows aggregated stats across all rounds with mastery ring,
// delta badges, and round-by-round breakdown.
//
// STANDALONE: depends on react, motion/react, lucide-react,
//   MasteryRing + mastery-colors from parent folder,
//   session-stats from lib.
// ============================================================
...
export function AdaptiveCompletedScreen({
  allStats,
  completedRounds,
  masteryDeltas,
  topicSummary,
  ...
  return (
    ...
    >
      {/* Ambient glow */}
      <div
        className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[400px] rounded-full blur-3xl pointer-events-none"
        style={{
          background: `radial-gradient(circle, ${masteryColor.hex}33 0%, ${masteryColor.hex}1A 50%, transparent 100%)`,
        }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center max-w-md">
        ...
        <p className="text-sm text-gray-500 mb-6">
          ...
          {aiRounds > 0 && (
            <span className="text-violet-500"> ({profRounds} profesor + {aiRounds} IA)</span>
          )}
        </p>
        ...
        <div className="mb-8">
          ...
          />
          <p className="text-xs text-gray-500 mt-2" style={{ fontWeight: 500 }}>
            Dominio general
          </p>
        </div>

        {/* Delta badges (shared component) */}
        {deltaStats && (
          <div className="mb-8">
            <DeltaBadges
        ...
        {/* Round history (shared component) */}
        {completedRounds.length > 1 && (
          <div className="mb-8 w-full max-w-xs">
            <RoundHistoryList rounds={completedRounds} />
          </div>
        )}
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={onRestart}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-600 hover:bg-white/80 transition-colors"
          ...
          <button
            onClick={onExit}
            className="px-7 py-2.5 rounded-xl bg-[#2a8c7a] text-white text-sm shadow-lg shadow-[#2a8c7a]/25 hover:bg-[#244e47] transition-colors"
