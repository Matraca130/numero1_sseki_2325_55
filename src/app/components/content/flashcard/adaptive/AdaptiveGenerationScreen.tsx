...
export function AdaptiveGenerationScreen({ progress, onCancel }: AdaptiveGenerationScreenProps) {
  ...
  useEffect(() => { setIsSlow(false); const timer = setTimeout(() => setIsSlow(true), SLOW_THRESHOLD_MS); return () => clearTimeout(timer); }, []);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center h-full bg-surface-dashboard p-8 text-center relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center max-w-md">
        <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} className="w-16 h-16 rounded-full bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center mb-6 shadow-xl shadow-teal-500/25"><Sparkles size={32} className="text-white" /></motion.div>
        ...
        <div className="relative mb-8" style={{ width: ringSize, height: ringSize }}>
          <svg width={ringSize} height={ringSize} className="-rotate-90"><circle cx={ringSize/2} cy={ringSize/2} r={ringRadius} fill="none" stroke="#e2e8f0" strokeWidth={ringStroke} /><motion.circle cx={ringSize/2} cy={ringSize/2} r={ringRadius} fill="none" stroke="#14b8a6" strokeWidth={ringStroke} strokeLinecap="round" strokeDasharray={ringCirc} animate={{ strokeDashoffset: ringCirc * (1 - pct / 100) }} transition={{ duration: 0.4, ease: 'easeOut' }} /></svg>
          <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-3xl text-gray-900 tabular-nums" style={{ fontWeight: 700 }}>{generated}</span><span className="text-xs text-gray-500" style={{ fontWeight: 500 }}>de {total}</span></div>
        </div>
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-4"><Loader2 size={14} className="animate-spin text-teal-500" /><span>{total === 0 ? 'Preparando generaci\u00F3n...' : completed < total ? `Creando flashcard ${completed + 1} de ${total}...` : `\u00A1${generated} flashcards creadas!`}</span></div>
        ...
        {isSlow && completed < total && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xs text-gray-400 mt-2">Esto puede tardar un momento &mdash; la IA est{'\u00E1'} analizando tu perfil de estudio</motion.p>}
        {onCancel && completed < total && <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} onClick={onCancel} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-500 hover:text-gray-700 hover:bg-white/60 transition-colors" style={{ fontWeight: 500 }}><XCircle size={14} />Cancelar generaci{'\u00F3'}n</motion.button>}
      </div>
    </motion.div>
  );
}
