  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center h-full bg-surface-dashboard p-8 text-center relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center max-w-md">
        <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} className="w-16 h-16 rounded-full bg-teal-50 flex items-center justify-center mb-6 shadow-xl shadow-[#2a8c7a]/25">
        ...
        {onCancel && completed < total && <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} onClick={onCancel} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-full border border-gray-200 text-sm text-gray-500 hover:text-gray-700 hover:bg-white/60 transition-colors" style={{ fontWeight: 500 }}><XCircle size={14} />Cancelar</motion.button>}
      </div>
    </motion.div>
  );
}
