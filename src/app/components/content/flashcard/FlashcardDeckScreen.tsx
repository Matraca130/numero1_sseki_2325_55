    ...
    >
      ...
      <div className="bg-white border-b border-gray-200/80">
        <div className="px-4 sm:px-6 md:px-8 pt-4 sm:pt-6 pb-4 sm:pb-5">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            {/* Desktop: all buttons inline. Mobile: only secondary actions */}
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={onStudyTopic}
                className="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-xl text-xs font-medium text-gray-600 transition-colors border border-gray-200"
              >
              ...
              {cards.length > 0 && (
                <button
                  onClick={() => onStart(cardsToStart)}
                  className="hidden sm:flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-sm shadow-sm hover:scale-105 hover:brightness-90 active:scale-95 transition-all"
                  style={{ backgroundColor: deckColor.hex, fontWeight: 700 }}
                >
                  <Play size={16} fill="currentColor" />
                  Estudiar{filterMastery !== 'all' ? ` (${filteredCards.length})` : ''}
                </button>
              )}
              ...
              {onStartAdaptive && cards.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl text-white text-xs sm:text-sm shadow-sm hover:scale-105 hover:brightness-90 active:scale-95 transition-all bg-gradient-to-r from-violet-500 to-[#2a8c7a]"
                  style={{ fontWeight: 600 }}
                  title="Sesi\u00F3n adaptativa con IA"
                >
                  <Sparkles size={14} />
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 bg-gray-50/80 rounded-xl border border-gray-100">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 flex-1 min-w-0">
                <span className="text-xs text-gray-600" style={{ fontWeight: 500 }}>
                  Keywords: <span style={{ fontWeight: 700 }} className="text-gray-800">{keywordProgress.keywordsMastered}/{keywordProgress.keywordsTotal}</span>
                </span>
                <div className="w-16 sm:w-24 h-1.5 rounded-full overflow-hidden bg-gray-200">
                  <div
                    className="h-full rounded-full transition-all duration-500"
                    style={{
                      width: `${Math.round(keywordProgress.overallMastery * 100)}%`,
                      backgroundColor: keywordProgress.overallMastery >= 0.75 ? '#10b981' : keywordProgress.overallMastery >= 0.4 ? '#f59e0b' : '#f43f5e',
                    }}
                  />
                </div>
                <span className="text-[11px] text-gray-400" style={{ fontWeight: 500 }}>
                  {Math.round(keywordProgress.overallMastery * 100)}%
                </span>
              </div>
              {keywordProgress.weakestKeywordName && keywordProgress.keywordsMastered < keywordProgress.keywordsTotal && (
                <span className="text-[10px] text-gray-400 truncate max-w-[140px] sm:max-w-[200px] hidden sm:inline" title={`Reforzar: ${keywordProgress.weakestKeywordName}`}>
                  Reforzar: <span className="text-amber-600" style={{ fontWeight: 500 }}>{keywordProgress.weakestKeywordName}</span>
                </span>
              )}
              {/* FSRS Coverage stats */}
              {keywordProgress.fsrsCoverage && keywordProgress.fsrsCoverage.totalMapped > 0 && (
                <div className="hidden sm:flex items-center gap-2 ml-auto border-l border-gray-200 pl-3">
                  {keywordProgress.fsrsCoverage.dueCards > 0 && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-50 text-amber-600" style={{ fontWeight: 600 }}>
                      {keywordProgress.fsrsCoverage.dueCards} pendientes
                    </span>
                  )}
                  {keywordProgress.fsrsCoverage.newCards > 0 && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-50 text-rose-500" style={{ fontWeight: 600 }}>
                      {keywordProgress.fsrsCoverage.newCards} nuevas
                    </span>
                  )}
                  <span className="text-[10px] text-gray-400" style={{ fontWeight: 500 }}>
                    {Math.round(keywordProgress.fsrsCoverage.coverage * 100)}% cobertura
                  </span>
                </div>
              )}
            </div>
          ...
          {filteredCards.length === 0 ? (
            ...
            <div className={CARD_GRID_CLASSES}>
              {filteredCards.map((card, idx) => (
                <FlashcardMiniCard key={card.id} card={card} idx={idx} onClick={() => onStart([card])} />
              ))}
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full text-left px-3.5 sm:px-5 py-3 sm:py-3.5 flex items-center gap-2.5 sm:gap-3.5 hover:bg-zinc-50/50 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2a8c7a]/30 focus-visible:ring-inset"
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-xs sm:text-sm text-zinc-800 truncate" style={{ fontWeight: 600 }}>
                          {group.label}
                        </h4>
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        {groupStats.newCards > 0 && (
                          <span className="hidden sm:inline text-[10px] px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-full" style={{ fontWeight: 600 }}>
                            {groupStats.newCards} pendientes
                          </span>
                        )}
                        <span className="text-[10px] sm:text-[11px] text-zinc-400" style={{ fontWeight: 500 }}>
                          {groupMastery}%
                        </span>

                        <div className="w-10 sm:w-16 h-1.5 rounded-full overflow-hidden bg-zinc-100">
                          <div
                            className="h-full rounded-full transition-all duration-500"
                            style={{
                              width: `${groupMastery}%`,
      ...
      {cards.length > 0 && (
        <div className="sm:hidden shrink-0 px-4 py-3 bg-white border-t border-gray-200 safe-area-bottom">
          <button
            onClick={() => onStart(cardsToStart)}
            className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-xl text-white text-sm shadow-lg active:scale-[0.98] transition-all"
            style={{ backgroundColor: deckColor.hex, fontWeight: 700 }}
          >
            <Play size={16} fill="currentColor" />
            Estudiar{filterMastery !== 'all' ? ` (${filteredCards.length})` : ` (${cards.length})`}
          </button>
        </div>
      )}
    </motion.div>
