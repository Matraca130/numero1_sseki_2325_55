...
export function AdaptiveFlashcardView() {
  ...
  // Registers a beforeunload listener when the session is in an active
  // phase (reviewing, partial-summary, generating). Does NOT fire in
  // idle (no data to lose) or completed (batch already submitted).
  // NOTE: Safari mobile ignores beforeunload — the window.confirm in
  // handleBack covers intentional navigation. Tab close on mobile is
  ...
  const handleBack = useCallback(() => {
    if (session.phase === 'reviewing' || session.phase === 'partial-summary' || session.phase === 'generating') {
      // In active session: confirm before leaving
