...
function FlashcardsSession({ onBack, topic, courseColor, accentColor }: any) {
    ...
    const closeSession = useCallback(() => {
      ...
      updateStudySession(sessionId, {
        ...
        total_reviews: sessionRatings.length,
    ...
    const handleRate = (level: number) => {
        ...
        if (backendSessionId.current && card) {
          submitReview({
            ...
            grade: Math.max(0, Math.min(5, level)),
          }).catch(err => {
...
// ── Alias for the new route architecture ──
// In the repo, this file becomes the thin delegate (StudyView → StudentSummariesView)
// and ActiveStudySessionView.tsx gets this full session content.
export { StudyView as ActiveStudySessionView };