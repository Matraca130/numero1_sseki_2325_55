            ...
            <div className="flex items-center justify-between mb-6 border-b border-gray-100 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-medium">Lista de temas disponibles</p>
                </div>
              </div>
              <button 
                onClick={(e) => { e.stopPropagation(); onCollapse(); }}
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {getLessonsForTopic(topic.id).length > 0 ? (
                          <span className="text-[10px] font-semibold text-teal-500 uppercase tracking-wider flex items-center gap-1">
                            <Layers size={9} />
                            {getLessonsForTopic(topic.id).length} Temas
                          </span>
                        ) : (
                          <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Resumen</span>
                        )}
                      </div>
                          ...
                          return (
                            <>
                              <span className="flex items-center gap-1">
                                <Clock size={11} /> 
                                {topicLessons.length > 0 ? `${topicLessons.length} temas` : '15 min'}
                              </span>
                              <span className="flex items-center gap-1">
                                <CheckCircle2 size={11} /> 
                                {completedLessons > 0 ? `${completedLessons}/${topicLessons.length}` : 'No iniciado'}
                              </span>
                            </>
                          );
                        })()}
...
export function StudyHubView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const states = await getFsrsStates({ limit: 500 });
        if (!cancelled) {
          const now = new Date();
          setPendingReviews(states.filter(s => s.due && new Date(s.due) <= now).length);
        }
      } catch { /* ignore */ }
    })();
      ...
      <AxonPageHeader
        ...
        statsLeft={
          <div className="flex items-center gap-4 text-sm text-gray-500">
            <span>{totalSections} secciones &middot; {totalTopics} temas</span>
            {isConnected && (
              <>
                {activePlans > 0 && (
                  <span className="flex items-center gap-1 text-teal-600">
                    <CalendarCheck size={13} /> {activePlans} {activePlans === 1 ? 'plan' : 'planes'}
                  </span>
                )}
                {pendingReviews > 0 && (
                  <span className="flex items-center gap-1 text-amber-600">
                    <Zap size={13} /> {pendingReviews} revisiones
                  </span>
                )}
                {avgMastery > 0 && (
                  <span className="flex items-center gap-1 text-emerald-600">
                    <Brain size={13} /> {avgMastery}% dominio
                  </span>
                )}
              </>
            )}
        ...
        actionButton={
          <button
            onClick={() => navigateTo('study-session')}
          ...
          >
            <PlayCircle size={15} /> Continuar Estudiando
          </button>
        }
      />
