...
function computeNodeStyle(
  node: import('@/app/types/mindmap').MapNode,
  isCollapsed: boolean,
  childCount: number,
...
function computeEdgeStyle(edge: import('@/app/types/mindmap').GraphData['edges'][number]) {
  return {
    id: edge.id,
    source: edge.source,
    target: edge.target,
...
export const KnowledgeGraph = memo(function KnowledgeGraph({
  ...
  // Kept as a callback because collapsed/positions are runtime arguments from effects.
  ...
  useEffect(() => {
    ...
    const clearActiveState = (evt: G6NodeEvent) => {
      ...
      const nodeId = evt.target?.id ?? evt.itemId;
      if (nodeId) {
        // Only remove 'active' state — preserve other states (hover, selected, multiSelected)
        try {
