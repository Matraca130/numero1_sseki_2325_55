...
export function PdfUploadButton({ institutionId, topicId, onSuccess, variant = 'default', size = 'sm', className = '' }: PdfUploadButtonProps) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <Button variant={variant} size={size} onClick={() => setOpen(true)} className={`gap-1.5 ${variant === 'default' ? 'bg-teal-500 hover:bg-teal-600 text-white' : ''} ${className}`}>
