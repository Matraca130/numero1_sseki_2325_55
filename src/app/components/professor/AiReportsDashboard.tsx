        ...
        <>
            ...
            {showStats && stats && (
              ...
              >
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <StatCard label="Total reportes" value={stats.total_reports} icon={<Flag size={15} />} color="#f59e0b" />
                  <StatCard label="Pendientes" value={stats.pending_count} icon={<Clock size={15} />} color="#ef4444" subValue={stats.reviewed_count > 0 ? `${stats.reviewed_count} en revision` : undefined} />
                  <StatCard label="Resueltos" value={stats.resolved_count} icon={<CheckCircle2 size={15} />} color="#22c55e" subValue={stats.dismissed_count > 0 ? `${stats.dismissed_count} descartados` : undefined} />
                  <StatCard label="Tasa resolucion" value={`${Math.round((stats.resolution_rate || 0) * 100)}%`} icon={<BarChart3 size={15} />} color="#8b5cf6" subValue={stats.avg_resolution_hours > 0 ? `~${Math.round(stats.avg_resolution_hours)}h promedio` : undefined} />
                </div>
                {reasonBreakdown.length > 0 && (
                  <div className="flex items-center gap-2 flex-wrap px-1">
                    <span className="text-[9px] text-zinc-400" style={{ fontWeight: 600 }}>Por motivo:</span>
                    {reasonBreakdown.map(({ reason, count, color }) => (
                      <span key={reason} className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] border" style={{ fontWeight: 600, color, backgroundColor: `${color}10`, borderColor: `${color}30` }}>
          ...
          <div className="border-t border-zinc-100 pt-3 mt-2">
            <div className="flex items-center justify-between mb-3">
              <button onClick={() => setShowList(!showList)} className="flex items-center gap-1.5 text-[11px] text-zinc-500 hover:text-zinc-700 transition-colors" style={{ fontWeight: 600 }}>
                <MessageSquare size={12} /> Reportes ({reportsTotal}) {showList ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
              </button>
              <button onClick={() => setShowFilters(!showFilters)} className={clsx('flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-colors border', (filterStatus || filterReason) ? 'text-purple-600 bg-purple-50 border-purple-200' : 'text-zinc-400 hover:text-zinc-600 border-transparent hover:border-zinc-200')} style={{ fontWeight: 600 }}>
                <Filter size={10} /> Filtros {(filterStatus || filterReason) && ' activos'}
              </button>
            </div>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="flex items-center gap-3 mb-3 pb-3 border-b border-zinc-100">
                    <div className="flex items-center gap-1.5">
                      ...
                      <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as AiReportStatus | '')} className="text-[11px] border border-zinc-200 rounded-lg px-2 py-1 bg-white text-zinc-700 focus:outline-none focus:ring-2 focus:ring-purple-300/50">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <select value={filterReason} onChange={(e) => setFilterReason(e.target.value as AiReportReason | '')} className="text-[11px] border border-zinc-200 rounded-lg px-2 py-1 bg-white text-zinc-700 focus:outline-none focus:ring-2 focus:ring-purple-300/50">
                        <option value="">Todos</option>
                    ...
                    {(filterStatus || filterReason) && (
                      <button onClick={() => { setFilterStatus(''); setFilterReason(''); }} className="text-[10px] text-zinc-400 hover:text-zinc-600 underline">Limpiar</button>
                    )}
                  </div>
                </motion.div>
              )}
            ...
            <AnimatePresence initial={false}>
              {showList && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  {reportsLoading ? (
                    <div className="flex items-center justify-center py-6"><Loader2 className="animate-spin text-purple-400" size={18} /></div>
                  ) : reports.length === 0 ? (
                    <p className="text-center text-[11px] text-zinc-400 py-6">{(filterStatus || filterReason) ? 'Sin reportes para estos filtros' : 'Sin reportes'}</p>
                  ) : (
                    <div className="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar-light pr-1">
                      {reports.map((report) => (<ReportRow key={report.id} report={report} resolving={resolvingId === report.id} onResolve={handleResolve} />))}
                    </div>
                  )}
                </motion.div>
              )}
            ...
            {reportsTotal > PAGE_SIZE && (
              <div className="flex items-center justify-between pt-2 mt-2 border-t border-zinc-100">
                <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0 || reportsLoading} className="text-[10px] text-zinc-400 hover:text-zinc-600 disabled:opacity-30 px-2 py-1 rounded hover:bg-zinc-50 transition-colors" style={{ fontWeight: 600 }}>Anterior</button>
                <span className="text-[9px] text-zinc-400">{page * PAGE_SIZE + 1}&ndash;{Math.min((page + 1) * PAGE_SIZE, reportsTotal)} de {reportsTotal}</span>
                <button onClick={() => setPage(p => p + 1)} disabled={(page + 1) * PAGE_SIZE >= reportsTotal || reportsLoading} className="text-[10px] text-zinc-400 hover:text-zinc-600 disabled:opacity-30 px-2 py-1 rounded hover:bg-zinc-50 transition-colors" style={{ fontWeight: 600 }}>Siguiente</button>
              </div>
            )}
          </div>
        </>
