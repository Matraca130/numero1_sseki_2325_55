...
import type { LeaderboardResponse, LeaderboardEntry, LeaderboardPeriod, BadgesResponse, BadgeCategory } from '@/app/types/gamification';

const CAT_COLORS: Record<BadgeCategory, string> = { consistency: '#f59e0b', study: '#3b82f6', mastery: '#8b5cf6', exploration: '#10b981', social: '#ec4899' };
const CAT_LABELS: Record<BadgeCategory, string> = { consistency: 'Consistencia', study: 'Estudio', mastery: 'Maestria', exploration: 'Exploracion', social: 'Social' };
...
export function ProfessorGamificationPage() {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      const [r1, r2] = await Promise.allSettled([getLeaderboard(iid, period, { limit: 20 }), getBadges()]);
  ...
  const catData = useMemo(() => {
    ...
    return Object.entries(m).map(([c, d]) => ({ name: CAT_LABELS[c as BadgeCategory] ?? c, total: d.t, earned: d.e, fill: CAT_COLORS[c as BadgeCategory] ?? '#6b7280' }));
  }, [bd]);
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        : (<>
          <div className="grid grid-cols-4 gap-4 mb-8">
            <SC icon={<Users size={16} />} label="Estudiantes activos" value={String(n)} color="purple" />
            <SC icon={<TrendingUp size={16} />} label="XP promedio" value={`${avgXp.toLocaleString()}`} color="blue" />
            <SC icon={<Award size={16} />} label="Nivel promedio" value={avgLvl} color="amber" />
            <SC icon={<Trophy size={16} />} label="Insignias" value={String(bd?.total ?? 0)} color="emerald" />
          </div>
