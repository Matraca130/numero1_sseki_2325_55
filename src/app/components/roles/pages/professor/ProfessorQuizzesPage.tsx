...
export function ProfessorQuizzesPage() {
  ...
  const {
    selectedSummaryId, selectedSummary, keywords,
    getKeywordName, cascadeLevels, breadcrumbItems,
  } = useQuizCascade();
  ...
  useEffect(() => { setFilterKeywordId(''); }, [selectedSummaryId]);

  const loadQuestions = useCallback(async () => {
    if (!selectedSummaryId) { setQuestions([]); return; }
    setQuestionsLoading(true);
    try {
      const filters: { question_type?: QuestionType; difficulty?: number; keyword_id?: string; limit?: number } = {};
  ...
  const filteredQuestions = useMemo(() => {
    ...
    return questions.filter(qq => qq.question.toLowerCase().includes(q) || qq.correct_answer.toLowerCase().includes(q));
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="shrink-0 bg-white border-r border-gray-100 flex flex-col overflow-hidden w-[300px]">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="px-4 py-3 border-b border-gray-100 w-full text-left hover:bg-gray-50/50 transition-colors cursor-pointer">
          <div className="flex items-center gap-2 mb-1">
            <ClipboardList size={16} className="text-purple-600" />
            <h2 className="text-sm text-gray-900" style={{ fontWeight: 700 }}>Quizzes</h2>
            <div className="flex-1" />
            <span className="text-gray-400 transition-transform">{sidebarOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</span>
          </div>
              ...
              <div className="flex-1 overflow-y-auto py-1 px-3">
                ...
                {selectedSummaryId && (
                  <div className="mt-2 px-3 py-2.5 rounded-lg bg-purple-50 border border-purple-100">
                    <div className="flex items-center gap-1.5 text-[10px] text-purple-600 mb-1" style={{ fontWeight: 700 }}><Check size={12} /> Resumen seleccionado</div>
                    <p className="text-[11px] text-purple-800 truncate" style={{ fontWeight: 500 }}>{selectedSummary?.title || selectedSummaryId.substring(0, 12)}</p>
                  </div>
                )}
                {institutionId && (
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-zinc-50">
        {!selectedSummaryId ? (
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-400 gap-3">
            <div className="w-16 h-16 rounded-2xl bg-purple-50 flex items-center justify-center"><ClipboardList size={28} className="text-purple-300" /></div>
          ...
          <>
            <div className="bg-white/80 backdrop-blur-xl border-b border-zinc-200 px-5 py-3">
              <Breadcrumb items={breadcrumbItems} className="mb-1.5 text-[10px]" />
              <div className="flex items-center gap-2">
                <FileText size={14} className="text-purple-500" />
                <span className="text-[13px] text-gray-800" style={{ fontWeight: 600 }}>{selectedSummary?.title || 'Resumen seleccionado'}</span>
                <span className="text-[10px] text-gray-400 ml-1">({selectedSummary?.status || ''})</span>
              </div>
            </div>
            <QuizStatsBar stats={stats} />
            <QuizFiltersBar filterType={filterType} filterDifficulty={filterDifficulty} filterKeywordId={filterKeywordId} searchQuery={searchQuery} keywords={keywords} onFilterTypeChange={setFilterType} onFilterDifficultyChange={setFilterDifficulty} onFilterKeywordChange={setFilterKeywordId} onSearchChange={setSearchQuery} onCreate={crud.handleCreate} />
            <div className="flex-1 overflow-y-auto custom-scrollbar-light p-5">
              {questionsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-gray-400 gap-2">
                  ...
                  <p className="text-sm">{questions.length === 0 ? 'No hay preguntas en este resumen' : 'Sin resultados para los filtros aplicados'}</p>
                  {questions.length === 0 && (
                    <button onClick={crud.handleCreate} className="mt-2 flex items-center gap-1.5 px-3 py-1.5 bg-purple-100 text-purple-700 rounded-lg text-[12px] hover:bg-purple-200 transition-colors" style={{ fontWeight: 600 }}><Plus size={14} /> Crear primera pregunta</button>
                  )}
                </div>
              ) : (
                <div className="space-y-3 max-w-4xl">
                  {filteredQuestions.map((q, idx) => (
                    <QuestionCard key={q.id} question={q} index={idx + 1} keywordName={getKeywordName(q.keyword_id)} onEdit={() => crud.handleEdit(q)} onDelete={() => crud.handleDelete(q.id)} onRestore={() => crud.handleRestore(q.id)} />
                  ))}
                </div>
              )}
            </div>
      ...
      <AnimatePresence>
        {crud.showModal && selectedSummaryId && (
          <QuestionFormModal summaryId={selectedSummaryId} question={crud.editingQuestion} keywords={keywords} onClose={crud.handleCloseModal} onSaved={crud.handleSaved} />
        )}
      </AnimatePresence>
    </div>
  );
