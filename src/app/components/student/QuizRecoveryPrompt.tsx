...
//
// Full-page prompt shown when localStorage backup is detected.
// Displays: quiz title, answered count, progress preview,
// time since last save. User can continue or start fresh.
//
// Design: teal accent, spring animation, matches quiz language.
// ============================================================
...
export function QuizRecoveryPrompt({ backup, onAccept, onDismiss, onBack }: QuizRecoveryPromptProps) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="flex flex-col items-center justify-center h-full p-8 bg-zinc-50"
    >
      <motion.div
      ...
      >
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-zinc-100 overflow-hidden">
          ...
          <div className="px-6 py-5 space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4 text-[12px] text-zinc-500">
                <span className="flex items-center gap-1.5">
                  <PlayCircle size={13} className="text-teal-500" />
                  ...
                  return (
                    <div
                      ...
                      className={`h-2 flex-1 rounded-full transition-colors ${
                        sa?.answered && sa.correct ? 'bg-emerald-400' :
                        sa?.answered && !sa.correct ? 'bg-rose-400' :
                        'bg-zinc-200'
                      }`}
                    />
                  );
            ...
            <div className="flex gap-3 pt-1">
              <button
                onClick={onDismiss}
                className="flex-1 py-2.5 rounded-xl border border-zinc-200 text-zinc-500 hover:bg-zinc-50 hover:text-zinc-700 transition-all text-sm"
              ...
              <button
                onClick={onAccept}
                className="flex-1 py-2.5 rounded-xl text-white bg-teal-600 hover:bg-teal-700 shadow-lg shadow-teal-600/25 transition-all text-sm inline-flex items-center justify-center gap-2"
        ...
        <button
          onClick={onBack}
          className="mt-4 mx-auto block text-sm text-zinc-400 hover:text-zinc-600 transition-colors"
