...
function BadgeItem({ badge, index }: { badge: Badge; index: number }) {
  ...
  // Use the icon field from backend (Lucide component name), fallback to Award
    ...
    >
      ...
      {earned && !shouldReduce && (
        <motion.div
          ...
          style={{
            boxShadow: `0 0 12px ${cat.glow}40`,
          }}
      ...
      {/* Rarity tag for earned rare+ badges */}
      ...
      {/* CSS hover tooltip fallback for desktop */}
      {!showTooltip && (
...
export function BadgeShowcase({ badges, isLoading, maxVisible = 12, className = '' }: BadgeShowcaseProps) {
  ...
  // Find closest-to-earning badge (highest progress/requirement ratio)
  ...
  return (
    <div className={`rounded-2xl border border-gray-200 bg-white p-4 ${className}`}>
      ...
      {/* Next badge to earn — motivational chase card */}
      {nextBadge && (
        <div
