...
//
// Minimal feed of recent XP transactions + badge awards.
// Uses GET /gamification/notifications.
// ============================================================
      ...
      <div className="space-y-1">
        {items.map((item, idx) => (
          <div
            ...
            className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg hover:bg-zinc-800/20 transition-colors"
          >
            {/* Icon */}
            {item.type === 'badge' ? (
            ...
            <span className="text-xs text-zinc-400 flex-1 truncate">
              {item.type === 'badge'
                ? item.badge_name || 'Logro desbloqueado'
                : ACTION_LABELS[item.action || ''] || item.action || 'XP ganada'
              }
            </span>

            {/* XP amount or badge rarity */}
            {item.type === 'xp' && item.xp && (
              <span className="text-[10px] text-amber-400/80 tabular-nums shrink-0" style={{ fontWeight: 600 }}>
                +{item.xp}
              </span>
            )}
            {item.type === 'badge' && item.badge_rarity && (
              <span className={`text-[9px] px-1.5 py-0.5 rounded-full capitalize shrink-0 ${
                item.badge_rarity === 'legendary' ? 'bg-amber-500/15 text-amber-400' :
                item.badge_rarity === 'epic' ? 'bg-violet-500/15 text-violet-400' :
