...
// Context: useGamification() for profile summary
// ============================================================

import React, { useState, useEffect, useCallback } from 'react';
import { motion } from 'motion/react';
import {
  History, Loader2, AlertTriangle, ChevronDown,
  Zap, Flame, Target, BookOpen, Brain, Award,
  CreditCard, CheckCircle, ListTodo,
} from 'lucide-react';
...
const ACTION_CONFIG: Record<string, { label: string; icon: typeof Zap; color: string }> = {
...
function getActionConfig(a: string) { return ACTION_CONFIG[a] ?? { label: a.replace(/_/g, ' '), icon: Zap, color: 'text-gray-500 bg-gray-50' }; }
...
export function XpHistoryPage() {
  ...
  const fetchPage = useCallback(async (offset: number, append: boolean) => {
    ...
    setError(null);
    try {
      const items: XpTransaction[] = await getXpHistory(institutionId, { limit: PAGE_SIZE, offset });
      if (append) setTransactions(prev => [...prev, ...items]); else setTransactions(items);
  ...
  const xpWeek = profile?.xp_this_week ?? 0;
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-green-100 flex items-center justify-center"><History size={22} className="text-green-600" /></div>
            <div><h1 className="text-xl text-gray-900" style={{ fontWeight: 800 }}>Historial de XP</h1><p className="text-[12px] text-gray-500">Tu actividad de experiencia</p></div>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3 mb-6">
          <SummaryCard label="Hoy" value={`${xpToday}`} icon={<Zap size={14} />} color="text-amber-600 bg-amber-50 border-amber-200" />
          <SummaryCard label="Esta semana" value={`${xpWeek}`} icon={<Flame size={14} />} color="text-orange-600 bg-orange-50 border-orange-200" />
          <SummaryCard label="Total" value={`${xpTotal.toLocaleString()}`} icon={<Award size={14} />} color="text-emerald-600 bg-emerald-50 border-emerald-200" />
        </div>
        ...
        ) : transactions.length === 0 ? (<div className="flex flex-col items-center justify-center py-20 text-gray-400"><History size={32} className="opacity-30 mb-3" /><p className="text-sm">Sin actividad registrada</p></div>
        ) : (<>
          <div className="space-y-1.5">{transactions.map((tx, i) => { const c = getActionConfig(tx.action); const I = c.icon; return (
            <motion.div key={tx.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.02, 0.2) }} className="flex items-center gap-3 px-4 py-3 bg-white rounded-xl border border-gray-100 hover:bg-gray-50 transition-colors">
              ...
              <div className="flex-1 min-w-0"><p className="text-[12px] text-gray-800" style={{ fontWeight: 600 }}>{c.label}</p>
                <div className="flex items-center gap-2"><span className="text-[10px] text-gray-400">{relativeTime(tx.created_at)}</span>
...
function SummaryCard({ label, value, icon, color }: { label: string; value: string; icon: React.ReactNode; color: string }) {
  return (<div className={`rounded-xl px-4 py-3 border ${color}`}>
    <div className="flex items-center gap-1.5 mb-1">{icon}<span className="text-[10px] uppercase tracking-wider opacity-70" style={{ fontWeight: 600 }}>{label}</span></div>
    <p className="text-lg tabular-nums" style={{ fontWeight: 800 }}>{value} XP</p>
  </div>);
}
