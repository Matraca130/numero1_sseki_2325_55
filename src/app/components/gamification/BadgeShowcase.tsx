...
//
// Displays earned vs locked badges in a clean grid.
// Earned badges glow subtly; locked ones are dimmed.
// ============================================================
...
interface BadgeShowcaseProps {
  ...
  /** Max badges to display (default: 12) */
  maxDisplay?: number;
}
...
export function BadgeShowcase({ institutionId, maxDisplay = 12 }: BadgeShowcaseProps) {
  ...
  useEffect(() => {
    ...
    gamificationApi.getBadges(institutionId)
      .then((res) => {
        setBadges(res.badges.slice(0, maxDisplay));
        setEarnedCount(res.earned_count);
      })
      .catch(() => {})
  ...
  if (badges.length === 0) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-xs text-zinc-600">Los logros se desbloquean al estudiar</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-zinc-400 uppercase tracking-wider" style={{ fontWeight: 600 }}>
            Logros
          </span>
        </div>
            ...
            <motion.button
              ...
              className={`
                ...
                ${isEarned
                  ...
                  : 'bg-zinc-800/20 border-zinc-800/40 opacity-40 hover:opacity-60'
                }
              `}
            >
              {/* Icon or lock */}
              ...
              <p className="text-[9px] text-zinc-400 text-center leading-tight truncate w-full">
                {badge.name}
              </p>
            </motion.button>
          );
      ...
      {/* Selected badge detail (expandable) */}
      {selectedBadge && (
        <motion.div
        ...
        >
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              ...
              {selectedBadge.earned && selectedBadge.earned_at && (
                <p className="text-[10px] text-zinc-600 mt-1">
                  Desbloqueado {new Date(selectedBadge.earned_at).toLocaleDateString('es')}
                </p>
              )}
            ...
            <span className={`text-[9px] px-1.5 py-0.5 rounded-full capitalize ${
              selectedBadge.rarity === 'legendary' ? 'bg-amber-500/15 text-amber-400' :
              selectedBadge.rarity === 'epic' ? 'bg-violet-500/15 text-violet-400' :
              selectedBadge.rarity === 'rare' ? 'bg-blue-500/15 text-blue-400' :
