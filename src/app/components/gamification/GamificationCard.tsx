...
export function GamificationCard() {
  ...
  // Cleanup timeout on unmount (BUG-FIX: prevent setState on unmounted)
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Fire daily check-in (idempotent) + onboarding (idempotent)
  ...
  // If no profile data yet (new student or endpoint unavailable)
  if (!profile) {
    return (
      <div className="bg-gradient-to-br from-amber-500/10 to-violet-500/10 border border-amber-500/20 rounded-xl p-5">
        ...
        <p className="text-xs text-zinc-400">
          Completa tu primera sesion de revision para empezar a ganar experiencia,
          subir de nivel y competir en el leaderboard.
        </p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="grid grid-cols-4 gap-2.5">
        ...
        <div className="bg-zinc-800/50 rounded-lg p-2.5 text-center">
          ...
          <p className="text-lg text-zinc-200 tabular-nums" style={{ fontWeight: 700 }}>
            {streak.current}
          </p>
      ...
      {leaderboard && leaderboard.leaderboard.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            {leaderboard.my_rank && (
              <span className="text-[10px] text-amber-400" style={{ fontWeight: 600 }}>
                Tu posicion: #{leaderboard.my_rank}
              </span>
            )}
          </div>
            ...
            {leaderboard.leaderboard.map((entry, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] text-amber-400 tabular-nums" style={{ fontWeight: 600 }}>
                    {weeklyXp} XP
                  </span>
                </div>
              );
      ...
      {/* ── Streak Actions (subtle) ── */}
      {(streak.current >= 2 || (streak.current === 0 && streak.longest > 0)) && (
        <div className="flex items-center gap-3 pt-1">
          {/* Buy freeze — available when streak is worth protecting */}
          {streak.current >= 2 && (
            <button
              ...
              onClick={async () => {
                ...
                if (result) {
                  ...
                  setActionMessage('No se pudo comprar (XP insuficiente?)');
                }
          ...
          {/* Repair streak — only when broken (current=0) */}
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (result && result.repaired) {
                  ...
                  setActionMessage('No elegible o XP insuficiente');
                }
                setActionPending(false);
                actionTimeoutRef.current = setTimeout(() => setActionMessage(null), 4000);
              }}
