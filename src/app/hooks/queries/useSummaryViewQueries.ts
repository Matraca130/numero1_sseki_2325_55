...
export function useReadingStateQuery(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ReadingState | null> => {
      ...
      if (Array.isArray(rs)) return rs[0] || null;
      if (rs && typeof rs === 'object' && 'items' in rs && Array.isArray((rs as Record<string, unknown>).items)) return ((rs as Record<string, unknown>).items as ReadingState[])[0] || null;
      return rs;
    },
