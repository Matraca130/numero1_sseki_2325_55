...
// Handles client-side validation, upload state, and result.
//
// BACKEND ROUTE:
//   POST /ai/ingest-pdf (Fase 7: ingest-pdf.ts)
//   - CONTENT_WRITE_ROLES only (professor/admin/owner)
//   - Max file size: 10MB
//   - Accepts: application/pdf only
//   - Pipeline: validate → Gemini extract → create summary → Storage
//              → fire-and-forget chunking + embedding
//
// PATTERN:
//   - Phase model: idle → validating → uploading → done/error
//   - Client-side validation before API call (fail fast)
//   - 60s timeout (Gemini extraction can be slow)
//   - Reset to upload another file
//
// DEPENDENCIES:
//   - aiService: ingestPdf
//   - Types: PdfIngestResponse
// ============================================================
