...
//
// PATTERN:
//   For count <= 10: single request with backend bulk mode
//     → 1 API call, backend does sequential Gemini, returns items[]
//   For count > 10: N individual calls with progress tracking
//     → N calls, client-side progress via onProgress
//
// BACKEND CONTRACT (generate-smart.ts, Fase 8A + 8E):
//   POST /ai/generate-smart
//   Body: { action, institution_id?, summary_id?, count?, quiz_id?, related? }
//   count=1: returns single item (flashcard or quiz_question row + _smart + _meta)
//   count>1: returns { items[], errors[], _meta }
//
// SUBTOPIC GRANULARITY:
//   The backend RPC get_smart_generate_target() returns targets with:
//   - subtopic_id + subtopic_name (when available)
//   - keyword_id + keyword_name (always)
//   - p_know (BKT mastery [0-1])
//   - need_score (composite score, higher = more need)
//   - primary_reason (new_concept | low_mastery | needs_review | ...)
//
//   Dedup is at subtopic level (Fase 8F): if a subtopic had AI content
//   generated in the last 2h, it's skipped. This ensures variety.
//
// DEPENDENCIES:
//   - aiService.generateSmart() (sends double-token, handles rate limit)
//   - SmartTargetMeta, SmartBulkResponse types from aiService.ts
// ============================================================
...
export function useSmartGeneration() {
  ...
  const generate = useCallback(async (params: SmartGenerationParams) => {
    ...
    try {
      // ── Strategy: Use backend bulk mode (count param) ──
      // Backend handles sequential Gemini calls + dedup internally.
      // For count <= 10, single request is more efficient:
      //   - Server-side dedup across the batch (no repeated subtopics)
      //   - 1 auth check instead of N
      //   - 1 context fetch instead of N
      // For count > 10, split into chunks of 10.
