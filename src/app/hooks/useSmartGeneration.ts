...
//
// Unified hook for smart AI generation of flashcards AND quiz questions.
// Uses POST /ai/generate-smart with subtopic-level granularity.
// ============================================================
...
export type SmartGenerationPhase = 'idle' | 'generating' | 'done' | 'error';
...
export function useSmartGeneration() {
  ...
  const generate = useCallback(async (params: SmartGenerationParams) => {
    ...
    setProgress({ completed: 0, total: count, generated: 0, failed: 0 });
    ...
    try {
      ...
      for (const chunkSize of chunks) {
        ...
        try {
          if (chunkSize === 1) {
            const singleResult = await generateSmart({ action, summaryId, institutionId, quizId, related });
            if (singleResult?.id) {
              allItems.push(mapSingleToItem(singleResult, action));
            }
          } else {
            const bulkResult = await generateSmart({ action, summaryId, institutionId, quizId, related, count: chunkSize }) as SmartBulkResponse;
            if (bulkResult?.items) {
            ...
            if (bulkResult?.errors) allErrors.push(...bulkResult.errors);
          }
        } catch (err: any) {
          allErrors.push({ keyword_id: 'unknown', keyword_name: 'Error de generacion', error: err?.message || 'Error desconocido' });
        }
      ...
      const subtopicIds = new Set(allItems.map(i => i.subtopic_id || i._smart?.target_subtopic).filter(Boolean));
      const avgPKnow = allItems.length > 0 ? allItems.reduce((sum, i) => sum + (i._smart?.p_know ?? 0), 0) / allItems.length : 0;

      const genResult: SmartGenerationResult = {
        items: allItems,
        errors: allErrors,
        stats: { requested: count, generated: allItems.length, failed: allErrors.length, uniqueKeywords: keywordIds.size, uniqueSubtopics: subtopicIds.size, avgPKnow, totalTokens: 0, elapsedMs },
      };
  ...
  const reset = useCallback(() => { setPhase('idle'); setProgress(null); setResult(null); setError(null); abortRef.current = false; }, []);

  return { phase, progress, result, error, generate, abort, reset };
}

function mapSingleToItem(raw: any, action: SmartGenerationAction): SmartGeneratedItem {
  return {
    id: raw.id, type: action, front: raw.front, back: raw.back,
    question: raw.question, options: raw.options, correct_answer: raw.correct_answer,
    explanation: raw.explanation, difficulty: raw.difficulty,
    ...
    _smart: raw._smart || { target_keyword: '', target_summary: '', target_subtopic: null, p_know: 0, need_score: 0, primary_reason: 'new_concept' as const },
  };
}
