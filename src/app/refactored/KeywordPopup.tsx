...
//
// REFACTORED (Phase A): Sections 1, 3, 4 delegated to sub-components.
// 43KB → ~13KB. All behavior preserved, zero API changes.
//
// Collapsible sections:
//   1. Definicion  → KeywordDefinitionSection (definition + prof/student notes)
//   2. Subtemas    → inline (small, ~40 lines)
//   3. Conexiones  → KeywordConnectionsSection (map + list + create)
//   4. Acciones    → KeywordActionsSection (counters + AI explain/chat)
//
// S1 migration: ALL data fetching via useKeywordPopupQueries (React Query).
...
// ── Sub-components (Phase A extraction) ───────────────────
...
export function KeywordPopup({
  ...
  const crossSummaryConnections = useMemo(() => {
    return connections
      ...
      .filter((x): x is { conn: any; ext: ExternalKeyword } => x !== null);
  ...
  }, [connections, keyword.id, localIds, kwName, keywordMasteryMap]);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto border-t border-zinc-800">
        ...
        {/* ─── 1. DEFINICION & NOTAS ──────────────────────── */}
        <CollapsibleSection
          icon={<BookOpen size={12} className="text-axon-accent" />}
          title="Definicion"
          badge={
        ...
        {/* ─── 2. SUBTEMAS (inline — small section) ───────── */}
        <CollapsibleSection
