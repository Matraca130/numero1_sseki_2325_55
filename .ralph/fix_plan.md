...
- [x] Round 40 audit (AiTutorPanel, ChangeHistoryPanel, changeHistoryHelpers): 4 fixes applied. (A) AiTutorPanel handleSuggestConnections — added suggestingRef concurrency guard (matching analyzingRef pattern). (B) AiTutorPanel handleAcceptSuggestion — fixed stale-closure race by using acceptedRef/acceptingKeyRef instead of state in deps (prevents duplicate edge creation on rapid clicks). (C) ChangeHistoryPanel TimelineEntry — added fallback values for ACTION_ICON/DOT_COLOR/ICON_BG/ICON_COLOR maps to prevent React crash on unknown entry.type. (D) changeHistoryHelpers loadHistory — added isValidEntry schema validation to filter malformed sessionStorage entries. Build clean, pushed
